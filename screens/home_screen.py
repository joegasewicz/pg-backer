...
class HomeScreen(wx.Frame):
    ...
    def __init__(self, parent=None, title="PG Backer"):
        ...
        # File Picker Field
        label_file = wx.StaticText(self.panel, label="Dump File")
        self.file_sizer = wx.BoxSizer(wx.HORIZONTAL)  # Horizontal sizer for input button
        self.input_file = wx.TextCtrl(self.panel, style=wx.TE_READONLY)
        self.browser_button = wx.Button(self.panel, label="Browse")
        ...
        # Log Panel
        self.log_output = LogPanel(self.panel)
        self.main_sizer.Add(self.log_output, flag=wx.ALL | wx.EXPAND, border=10, proportion=1)

        # Buttons
        self.dump_button = wx.Button(self.panel, label="Dump", size=(120, 40))
        self.cancel_button = wx.Button(self.panel, label="Cancel", size=(120, 40))
